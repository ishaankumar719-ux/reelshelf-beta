...
function PosterCard({ show }: { show: SeriesRow }) {
  const posterSrc = show.poster ? `https://image.tmdb.org/t/p/w185${show.poster}` : null

  return (
    <Link
      href={`/series/${show.media_id}`}
        ...
        >
          {posterSrc ? (
            ...
            <div style={{ width: "100%", height: "100%", display: "flex", alignItems: "center", justifyContent: "center", padding: 8 }}>
              <span style={{ fontSize: 10, color: "rgba(255,255,255,0.25)", textAlign: "center", lineHeight: 1.3 }}>{show.title}</span>
            </div>
          )}
        </div>
        {show.favourite && (
...
export default async function SeriesPage({ params }: { params: Promise<{ username: string }> }) {
  ...
  // Deduplicate by show (media_id), keeping the most recent entry
  const seen = new Set<string>()
  const shows: SeriesRow[] = []
  for (const row of (rows ?? []) as SeriesRow[]) {
    if (!seen.has(row.media_id)) {
    ...
    >
      {shows.length === 0 ? (
        <p style={{ fontSize: 13, color: "rgba(255,255,255,0.3)", fontStyle: "italic", padding: "24px 0" }}>
          {isOwner ? "You haven't logged any series yet." : `${displayName} hasn't logged any series yet.`}
        </p>
      ) : (
...
function PrivatePage({ username }: { username: string }) {
  return (
    <PageShell title="Series" subtitle={`@${username}`} backHref={`/u/${username}`} count={null} username={username}>
      <p style={{ fontSize: 13, color: "rgba(255,255,255,0.3)", fontStyle: "italic", padding: "24px 0" }}>
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#08080f", color: "rgba(255,255,255,0.85)" }}>
      <div style={{ maxWidth: 800, margin: "0 auto", padding: "24px 20px 80px" }}>
        ...
        <ProfileTabStrip username={username} activeTab="series" />
        {children}
      </div>
    </div>
  )
