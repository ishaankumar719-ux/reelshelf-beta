...
async function getOrCreateRotation(today: string): Promise<RotationRow | null> {
  ...
  // Create: pick random eligible questions per category (avoid last 30 days)
  const cutoff = new Date(today)
  cutoff.setDate(cutoff.getDate() - 30)
  const cutoffStr = cutoff.toISOString().split("T")[0]

  const { data: recentRot } = await admin
    .from("trivia_daily_rotation")
  ...
  const { error } = await admin.from("trivia_daily_rotation").insert(rotation)
  if (error) {
    ...
    const { data: fetched } = await admin
      ...
      .eq("rotation_date", today)
      .maybeSingle()
    return (fetched as RotationRow | null) ?? rotation
  }
...
async function getCommunityStats(
  ...
  if (!admin || questionIds.length === 0) return {}
...
export default async function TriviaPage() {
  ...
  const rotation = await getOrCreateRotation(today)
  ...
  let questions: Record<Category, TriviaQuestion | null> = { film: null, tv: null, book: null }
  ...
  const communityStatsMap = await getCommunityStats(admin, answeredIds)
